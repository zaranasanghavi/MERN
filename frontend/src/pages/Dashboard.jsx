...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        const { data: userData } = await axios.get("https://mern-jvn8.onrender.com/api/auth/user", {
          headers: { Authorization: `Bearer ${token}` },
        });
        setUser(userData);

        const { data: lostData } = await axios.get("https://mern-jvn8.onrender.com/api/lost/user/lostitems", {
          headers: { Authorization: `Bearer ${token}` },
        });
        setLostItems(lostData);

        const { data: foundData } = await axios.get("https://mern-jvn8.onrender.com/api/found/user/founditems", {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleLostItemUpdate = async () => {
    try {
      ...
      const response = await axios.put(
        `https://mern-jvn8.onrender.com/api/lost/${editForm._id}`,
        formData,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleFoundItemUpdate = async () => {
    try {
      ...
      const response = await axios.put(
        `https://mern-jvn8.onrender.com/api/found/${editFoundForm._id}`,
        formData,
        { headers: { Authorization: `Bearer ${token}` } }
      );
