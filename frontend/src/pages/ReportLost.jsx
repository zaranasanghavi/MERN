...
const ReportLost = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await axios.get("http://https://mern-jvn8.onrender.com/api/lost/categories");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post("http://https://mern-jvn8.onrender.com/api/lost/report", formData, {
        headers: {
    ...
    <div className="flex flex-col md:flex-row min-h-screen bg-gray-100">
      ...
      <div className="md:w-1/2 w-full bg-white p-6 md:p-10 flex flex-col justify-center">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button disabled={userId == ''} className="w-full bg-red-500 text-white p-2 rounded hover:bg-red-600">
            Submit Report
          </button>
        </form>
      </div>
